...
const oneShot_nominating_pending_payouts = async (task: SubscriptionTask) => {
  ...
  await Callbacks.callback_nominating_pending_payouts(data, entry);
};
...
const oneShot_nominating_exposure = async (task: SubscriptionTask) => {
  ...
  switch (chainId) {
    case 'Polkadot':
    case 'Kusama': {
      await Callbacks.callback_nominating_exposure(data, entry);
      break;
    }
    case 'Westend': {
      await Callbacks.callback_nominating_exposure_westend(data, entry);
      break;
    }
  }
};
